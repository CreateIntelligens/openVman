...
DEFAULT_ANSWER_RULES = (
    ...
    "兩者衝突時採知識庫的說法。\n"
    "4. 若問題涉及流程，給出清楚下一步。\n4a. 用使用者最新一句話的語言回答（講英文就回英文、講西班牙文就回西班牙文）；使用者講中文或語言無法判斷時用繁體中文。使用者明確指定語言時照指定。\n"
...
NO_TOOLS_ANSWER_RULES = (
    ...
    "用使用者最新一句話的語言回答，中文或無法判斷時用繁體中文，使用者指定語言時照指定；一律輸出純文字（Plain text），嚴禁使用 Markdown 格式（不可使用 ** 星號粗體、# 標題或列表符號），不要使用 emoji。"
)
