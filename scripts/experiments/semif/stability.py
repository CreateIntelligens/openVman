...
def summarize(rows: list[dict], metadata: dict) -> dict:
    actual_schedule = [
        [row["repeat"], row["order_index"], row["id"]] for row in rows
    ]
    assert actual_schedule == metadata["schedule"], "Execution schedule mismatch"
    assert len({tuple(job) for job in actual_schedule}) == len(rows)
    ...
    assert len(by_case) == 32 and set(by_order) == set(range(24))
    assert all({row["repeat"] for row in group} == set(range(REPEATS))
    ...
    return {
        "metadata": metadata,
        "summary_runner_sha256": hashlib.sha256(
        ...
        "first_repeat_order_sensitive_cases": sum(
            row["distinct_labels_across_orders"] > 1 for row in case_results
        ),
        "first_repeat_always_correct_cases": sum(
...
def main() -> None:
    ...
    metadata = {
        ...
        "note": (
            ...
            "Fixed RNG seed and shuffled job order; no answer sampling. Development fixture, "
