...
#   17 4 * * * /path/to/repo/scripts/renew-letsencrypt.sh >>/path/to/repo/backend/logs/letsencrypt-renew.log 2>&1
set -eu
...
nginx_pid="$(ps -eo pid=,args= | awk '$0 ~ /nginx: master process \/usr\/sbin\/nginx([[:space:]]|$)/ {print $1; exit}')"
if [ -n "$nginx_pid" ]; then
  "$DOCKER_BIN" run --rm --pid=host --privileged alpine:3.20 kill -HUP "$nginx_pid"
else
