...
describe("avatar api", () => {
  it("fetchAvatarCharacters GETs /api/v1/avatar", async () => {
  ...
  it("deleteAvatarCharacter DELETEs by id", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/avatar/008");
  ...
  it("renameAvatarCharacter POSTs new id", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/avatar/008/rename");
  ...
  it("fetchAvatarBackgrounds GETs /api/v1/backgrounds", async () => {
  ...
  it("uploadAvatarBackground POSTs multipart", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/backgrounds");
    expect((init as RequestInit).method).toBe("POST");
    expect((init as RequestInit).body).toBeInstanceOf(FormData);
  });
  ...
  it("deleteAvatarBackground DELETEs by id", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/backgrounds/clinic");
    expect((init as RequestInit).method).toBe("DELETE");
  });
  ...
  it("fetchAvatarMascots GETs /api/v1/avatar/mascots", async () => {
  ...
  it("uploadAvatarMascot POSTs multipart", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/avatar/mascots");
    expect((init as RequestInit).method).toBe("POST");
    expect((init as RequestInit).body).toBeInstanceOf(FormData);
  });
  ...
  it("deleteAvatarMascot DELETEs by id", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/avatar/mascots/custom");
    expect((init as RequestInit).method).toBe("DELETE");
  });
  ...
  it("uploadAvatarMascotThumbnail POSTs multipart thumbnail", async () => {
    ...
    const [url, init] = f.mock.calls[0];
    expect(url).toBe("/api/v1/avatar/mascots/custom/thumbnail");
    expect((init as RequestInit).method).toBe("POST");
    expect((init as RequestInit).body).toBeInstanceOf(FormData);
  });
});
