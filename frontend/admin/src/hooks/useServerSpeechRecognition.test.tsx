...
import { ASR_ERROR_MESSAGES } from "@shared/speech";
...
describe("useServerSpeechRecognition (admin hook)", () => {
  beforeEach(() => {
  ...
  it("等後端回字的期間 transcribing 為 true", async () => {
    ...
    await act(async () => {
      release(
        new Response(JSON.stringify({ text: "好", provider: "breeze" }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }),
      );
  ...
  it("後端的失敗佔位字不會被當成使用者說的話送出", async () => {
    ...
    await waitFor(() =>
      expect(onError).toHaveBeenCalledWith(ASR_ERROR_MESSAGES["transcribe-failed"]),
    );
  ...
  it("什麼都沒錄到就不上傳", async () => {
    ...
    await waitFor(() =>
      expect(onError).toHaveBeenCalledWith(ASR_ERROR_MESSAGES["no-speech"]),
    );
    expect(apiFetchMock).not.toHaveBeenCalled();
  });
  ...
  it("麥克風權限被拒時回報錯誤而不是卡在收音中", async () => {
    ...
    await waitFor(() =>
      expect(onError).toHaveBeenCalledWith(ASR_ERROR_MESSAGES["not-allowed"]),
    );
    expect(result.current.recording).toBe(false);
  });
});
