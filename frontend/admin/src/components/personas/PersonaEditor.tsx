      ...
      <div className="flex flex-col gap-6 mb-6 shrink-0">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar border-b border-slate-800/60 pb-3">
          {coreDocs.map((doc) => (
            ...
            >
              <span className={`material-symbols-outlined text-[16px] ${selectedPath === doc.path ? "text-slate-200" : ""}`}>
                {doc.icon}
              </span>
              {doc.label}
            </button>
      ...
      <div className="flex-1 min-h-0 relative mb-5 rounded-xl border border-slate-800/50 bg-slate-950/30 overflow-hidden shadow-inner flex">
        ...
        {editorMode === "edit" || editorMode === "split" ? (
          <textarea
            ...
            className={`h-full w-full bg-transparent p-6 text-[13px] leading-relaxed text-slate-200 placeholder:text-slate-600 focus:outline-none font-mono resize-none ${
              editorMode === "split" ? "border-r border-slate-800/50" : ""
            }`}
          />
        ) : null}
