...
export default function WorkspaceMobileSidebar({
  ...
  return (
    <div className="fixed inset-0 z-40 md:hidden" onClick={onClose}>
      ...
      <aside
        className="absolute inset-y-0 left-0 w-[18.75rem] border-r border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-950 flex flex-col"
      ...
      >
        <div className="px-5 py-5 border-b border-slate-200 dark:border-slate-800/60 flex items-center justify-between shrink-0 bg-slate-50 dark:bg-slate-900/20">
          ...
          <button
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors"
          >
