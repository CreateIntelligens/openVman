...
export default function MarkdownPreview({ content, className = "" }: MarkdownPreviewProps) {
  return (
    ...
    <Markdown
      components={{
        h1: ({ children }) => <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-4 mt-6 first:mt-0">{children}</h1>,
        h2: ({ children }) => <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 mt-5 border-b border-slate-200 dark:border-slate-800 pb-2">{children}</h2>,
        h3: ({ children }) => <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2 mt-4">{children}</h3>,
        p: ({ children }) => <p className="text-sm leading-7 text-slate-600 dark:text-slate-300 mb-4">{children}</p>,
        ...
        li: ({ children }) => <li className="text-sm text-slate-600 dark:text-slate-300">{children}</li>,
        ...
        blockquote: ({ children }) => <blockquote className="border-l-4 border-primary/40 pl-4 italic text-slate-500 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-primary/5 py-2 pr-4 rounded-r-lg">{children}</blockquote>,
        ...
        strong: ({ children }) => <strong className="font-bold text-slate-900 dark:text-slate-200">{children}</strong>,
        ...
        th: ({ children }) => <th className="border border-slate-200 dark:border-slate-700 px-4 py-2.5 text-left font-semibold text-slate-900 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/50">{children}</th>,
        td: ({ children }) => <td className="border border-slate-200 dark:border-slate-700 px-4 py-2 bg-white dark:bg-slate-900/20 text-slate-700 dark:text-slate-300">{children}</td>,
      }}
    >
      {content}
    </Markdown>
