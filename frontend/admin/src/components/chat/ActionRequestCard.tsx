...
export default function ActionRequestCard({
  ...
  return (
    <div className="mt-3 rounded-lg border border-border bg-surface-raised p-3 text-sm">
      ...
      <p className="mb-3 text-content-muted leading-relaxed">
        {request.description}
      </p>
      {request.reason && (
      ...
      {status === "error" && (
        <p className="mb-2 text-xs text-red-600">執行失敗：{errorMsg}</p>
      )}
      {status === "confirmed" && (
        <p className="text-xs text-accent-600">✓ 已觸發。結果將稍後顯示。</p>
      )}
      {status === "cancelled" && (
        <p className="text-xs text-content-subtle">已取消。</p>
      )}
      {(status === "pending" || status === "confirming" || status === "error") && (
