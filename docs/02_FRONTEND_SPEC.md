...
前端僅負責編輯 `.md` 文件。即使上傳的是 PDF、DOCX、PPTX 或 XLSX，後端也會先保留原始檔於 `workspace/raw/`；PDF 會先嘗試 pdf-inspector text-based fast path，其餘文件或 fallback 再透過 Docling / MarkItDown 轉換管線生成 Markdown 文件提供前端編輯，確保 AI 讀取的資料格式具備最高的一致性。
