...
* **可觀測性**：Pino (JSON Log), Prometheus (Metrics)
  > **實作現況**：以 Python logging (JSON structured) 實作，metrics 端點為 `GET /metrics`。
...
* **Response**:
  ...
  {
    "status": "ok",
    "dependencies": { "redis": "connected", "temp_storage": "ok", "brain": "ok", "index-tts": "healthy" },
    "stats": { "temp_usage_mb": 45, "temp_limit_mb": 5120 }
  }
  ```

  > **實作現況**：pdf-inspector / Docling 是 backend container 內的 Python dependencies，不是預設 downstream health probe；只有舊版 `DOCLING_SERVE_URL` 被設定時，health 才會額外探測外部 Docling service。
