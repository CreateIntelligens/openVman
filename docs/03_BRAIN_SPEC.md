...
可選的 [Embedding 意圖影子模式](intent-shadow.md)在普通使用者回合的 prompt 組裝後提交背景分類。分類僅寫入觀測記錄，不回傳至路由決策，也不修改工具、prompt 或強制檢索。控制訊息與明確指定工具的回合不參與；抽樣、單筆容量、HTTP timeout 與失敗冷卻限制額外工作。
