...
展示即時語音閉環。Backend 透過 `LiveVoicePipeline` 呼叫 Brain REST API 取得完整回覆，切句後依序送 TTS 合成（`TTSRouterService`：IndexTTS → VoxCPM → CosyVoice → Gemini → GCP → AWS → Edge-TTS）。
...
sequenceDiagram
    ...
    B->>B: 分類、保存原始檔與標準化
    Note over B: .md/.txt/.csv 直通；PDF 先嘗試 pdf-inspector；其餘 PDF / Office 文件走 Docling，失敗時 fallback 至 AnyDoc
