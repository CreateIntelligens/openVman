...
此功能只觀察，不控制正式 RAG。預設關閉；本次未修改正式環境的 .env，也未部署或重啟 Brain。
...
[獨立案例報告](../scripts/experiments/intent-shadow/REPORT.md)包含固定資料集、逐筆結果及實際服務 smoke 記錄。測試覆蓋抽樣、忙碌、冷卻、timeout、錯誤資料與身分漂移，以及正常／失敗時保留原聊天行為。
