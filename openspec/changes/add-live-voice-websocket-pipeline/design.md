...
The current codebase already contains the main ingredients for live voice interaction, but they are fragmented across layers. `frontend/app` has browser ASR and lip-sync service skeletons, `brain/api` can stream tokens over SSE, and `backend/app` can synthesize audio through `TTSRouterService`. The existing `/ws/{client_id}` handler is still a placeholder, the shared protocol is missing live control events, and frontend runtime wiring is incomplete.
