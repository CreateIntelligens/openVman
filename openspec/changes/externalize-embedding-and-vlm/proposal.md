...
- Add an optional Compose launcher that resolves required embedding plus explicitly selected optional GPU profiles, while keeping direct `docker compose` operation deterministic and supported.
...
- `gpu-service-routing`: External-URL precedence, local profile selection, launcher behavior, VLM and IndexTTS parity, nginx exposure, and failure reporting for reusable GPU services.
...
- Affected deployment code: `docker-compose.yml`, `.env.example`, nginx routing, a new embedding gateway image, and an optional Compose launcher.
