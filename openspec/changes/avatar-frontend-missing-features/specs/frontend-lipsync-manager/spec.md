...
#### Scenario: Live runtime advertises lip-sync mode via WebSocket
- **WHEN** the live runtime receives `server_init_ack`
- **THEN** the frontend SHALL immediately send `{ "event": "set_lip_sync_mode", "mode": "<active-mode>" }` so Backend can optimize live payload behavior
