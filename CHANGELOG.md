...
### Changed / Fixed
- **Decoupled Formal Account Creation and Rights Assignment**: The admin account creation form now only requires username, password, and role. Resource grants and defaults are decoupled into the account list's `FormalAccountAccessPanel`, which automatically opens upon creating a new user to streamline onboarding without blocking initial account creation.
